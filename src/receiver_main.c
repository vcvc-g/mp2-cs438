    ...
    while(1){
        ...
        /* ESTAB state, start writing file */
            ...
            if(recvBuffer[0] == 'M'){
                ...
                printf("cur_seq: %d, exp_seq: %d, ack_seq: %d\n", cur_seq, recvInfo->next_expected,recvInfo->last_ack);

                /*generate ACK*/
                ACK[0] = 'A';
                ACK[1] = (uint8_t)(recvInfo->last_ack/255);
                ACK[2] = (uint8_t)(recvInfo->last_ack%255);
                sentBytes = sendto(s, ACK, msg_total_size, 0, (struct sockaddr*)&si_other, slen);
